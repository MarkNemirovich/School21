...
void s21_read_flags(char **format, va_list list, flag *flags) {
  ...
  if (**format == '.') {
    ...
    if (**format == '*') {
      ...
      flags->accuracy = s21_get_num(format, 0);
  }
}
...
void s21_add_spaces(char *str, int *i, flag flags, char mode) {
  ...
  if (flags.hash && !flags.zeros && mode == 'o') {
    if (flags.width > 0) *i -= 1;  // если # и заполняли пробелами,
    str[(*i)++] = '0';  // то добавляем 0 перед цифрами
  }
  if (flags.hash && !flags.zeros && (mode == 'x' || mode == 'X')) {
    if (flags.width > 0) *i -= 2;
    str[(*i)++] = '0';  // если # и заполняли пробелами,
    str[(*i)++] = mode;  // то добавляем 0x / 0X перед цифрами
  }
}
