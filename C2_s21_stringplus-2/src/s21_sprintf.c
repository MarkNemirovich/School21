...
int s21_get_type(char *str, int *i, va_list list, char **format, flag flags) {
  ...
  if (**format == 'c')
    ...
    if (flags.longest)
      length = s21_get_f(str, i, va_arg(list, long double), flags);
    else
      length = s21_get_f(str, i, va_arg(list, double), flags);
  } else if (**format == 'e' || **format == 'E') {
    if (!flags.point) flags.accuracy = 6;
    if (flags.longest)
      length = s21_get_e(str, i, va_arg(list, long double), flags, **format);
    else
      length = s21_get_e(str, i, va_arg(list, double), flags, **format);
  } else if (**format == 'g' || **format == 'G') {
    if (flags.longest)
      length = s21_get_g(str, i, va_arg(list, long double), flags, **format);
    else
      length = s21_get_g(str, i, va_arg(list, double), flags, **format);
  } else if (**format == 'p')
...
int s21_ftoa(long double n, char *res, flag flags) {
  ...
  if (flags.accuracy > 0) {
    ...
    i += s21_int_to_str((long long)fpart, res + i, flags.accuracy, 10, '\0');
  }
  // end float part
  res[i] = '\0';
...
int s21_etoa(long double n, char *res, flag flags, char mode) {
  ...
  i += s21_int_to_str((long long)power, res + i, 2, 10, '\0');  // add power
  return i;
}
